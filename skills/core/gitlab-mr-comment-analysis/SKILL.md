...
description: >-
  ...
  (prefer `review_mr_<MR>.md`; use `analysis_mr_<MR>.md` when that file is the working artifact),
...
Put everything into **one** MR artifact:

1. Prefer **`review_mr_<MR>.md`** as the canonical combined bootstrap + grouped-comment workspace.
2. If the session uses **`analysis_mr_<MR>.md`** instead (investigation-heavy bootstrap or user-provided file only), enrich **that** file with the same grouped-comment sections—do not create a parallel `review_mr_<MR>.md` for grouped comments unless the user asks.

Resolve `<MR>` from live `gitlab` context (`mr_iid`) before naming paths.
...
- Open or create the **single main artifact** (`review_mr_<MR>.md` by preference; otherwise `analysis_mr_<MR>.md` per rules above). If missing, bootstrap minimal MR framing consistent with `../ARTIFACTS.md`, then continue.
...
- or an existing local MR artifact (`review_mr_<MR>.md` or `analysis_mr_<MR>.md`)
...
2. Resolve `mr_iid`, choose `review_mr_<MR>.md` vs `analysis_mr_<MR>.md` per **Single main artifact**.
...
11. Show an on-screen summary (2–3 lines per grouped issue, stable labels, path to the **single** main artifact).
...
Main artifact: review_mr_<MR>.md (or analysis_mr_<MR>.md if that is the session artifact).
...
- `review_mr_<MR>.md` **or** `analysis_mr_<MR>.md` (exactly one chosen main artifact per run series)
