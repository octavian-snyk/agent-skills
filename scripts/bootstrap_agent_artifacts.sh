...
Bootstrap the external agent-artifacts store and optional Cursor rule from
templates/ in this repository. Idempotent by default (skip existing files).
...
Options:
  ...
  --overwrite            Replace README and Cursor rule when templates differ
